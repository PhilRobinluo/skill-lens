...
**一个安装在本地的 Claude Code Skills 管理工具。**
...
技能透镜是一个**本地 Web 应用**，专门用来管理 Claude Code 的全局 Skills（`~/.claude/skills/`）。

它运行在你自己的电脑上（`localhost:3000`），扫描你本地的 Skill 文件，把散落在文件夹里的几十上百个 `.md` 文件变成一个可视化的仪表盘。**所有数据都在你本地，不上传任何内容，不需要联网就能使用核心功能。**
...
当你深度使用 Claude Code 之后，你会把越来越多的能力封装成 Skill — 自己的经验、别人的最佳实践、学到的方法论，统统装进去。慢慢地技能库膨胀到 50、80、130 个……然后问题来了：
...
你需要一个地方，能**一眼看清所有 Skill 的全貌** — 谁在被用、谁在吃灰、谁和谁功能重叠、CLAUDE.md 的路由有没有断裂。技能透镜就是干这个的。
...
| **自动扫描** | 启动时自动扫描 `~/.claude/skills/` 和插件目录，文件变动实时更新 |
